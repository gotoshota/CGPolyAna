...
contains
    ...
    ! =======================================================
    !subroutine write_lammpstrj(params, headers, filename)
    !    implicit none

    !    type(paramsectory), intent(in) :: params
    ...
    !        write(dump, *) params%timesteps(frame)
    !        write(dump, "(A)") "ITEM: NUMBER OF ATOMS"
    !        write(dump, *) params%nparticles
    !        if ( size(params%box_dim, 1) == 2) then
    !            write(dump, "(A)") "ITEM: BOX BOUNDS pp pp pp"
    !        else
    !            write(dump, "(A)") "ITEM: BOX BOUNDS xy xz yz pp pp pp"
    !        end if
    !        write(dump, *) (params%box_dim(i, 1, frame), i=1, size(params%box_dim, 1))
    !        write(dump, *) (params%box_dim(i, 2, frame), i=1, size(params%box_dim, 1))
    !        write(dump, *) (params%box_dim(i, 3, frame), i=1, size(params%box_dim, 1))
    !        write(dump, '(A)') trim(header_string)  ! Using '(A)' format to write the full header_string

    !        ! Write atom data
    !        do i = 1, params%nparticles
    ...
    !            if (headers%xu /= 0) write(dump, '(F8.3, x)', advance='no') params%coords(1, i, frame)
    !            if (headers%yu /= 0) write(dump, '(F8.3, x)', advance='no') params%coords(2, i, frame)
    !            if (headers%zu /= 0) write(dump, '(F8.3, x)', advance='no') params%coords(3, i, frame)
    !            if (headers%x /= 0) write(dump, '(F8.3, x)', advance='no') params%coords(1, i, frame)
    !            if (headers%y /= 0) write(dump, '(F8.3, x)', advance='no') params%coords(2, i, frame)
    !            if (headers%z /= 0) write(dump, '(F8.3, x)', advance='no') params%coords(3, i, frame)
